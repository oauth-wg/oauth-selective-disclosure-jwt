...
jws_sd_p_doc = {
    ...
    "sd": {
        name: hash_claim(salts[name], full_user_claims[name], return_raw=True) for name in disclosed_claims
    }
}

print ("Contents of the JWS-SD-Proof:\n```\n" + dumps(jws_sd_p_doc, indent=4) + "\n```\n\n")
...
print("The serialized JWS-SD-Proof:\n```\n" + jws_sd_p.serialize(compact=True) + "\n```\n\n")
