...
A popular application of JWS are JSON Web Tokens (JWT) [@!RFC7519], which are often used to represent a user's identity.
An ID Token as defined in OpenID Connect [@?OpenID.Core], for example, is a JWT containing the user's claims created by the server for consumption by a client.
In case the JWS is sent directly from the server to the client, as in OpenID Connect, the server can select which user claims to include in the JWT, minimizing the information shared with the client.

A new model is emerging that decouples the issuance of a JWT from its presentation.
In this model, a JWT containing many claims is issued to an intermediate party, who holds the JWT (the Holder).
The Holder, typically an end-user, can then present the JWT to different verifying parties (Verifiers), who each may only require a subset of the claims in the JWT.
For example, the JWT may contain claims representing both an address and a birthdate.
The Holder would like to disclose only their address to one Verifier, and only their birthdate to a different Verifier.

This model requires a mechanism enabling selective disclosure of data elements while ensuring that Verifiers can still check the authenticity of the data provided.
This specification defines such a mechanism for JSON-encoded payloads of JSON Web Signatures, with a primary use case being JSON Web Tokens.
...
For presentation to a Verifier, the Holder sends the signed payload along with with the cleartext of those claims it wants to disclose.
...
When Key Binding is enforced, a Holder has to proof possession of a private key belonging to a public key contained in the SD-JWT itself.
It usually does so by signing over a data structure containing transaction-specific data, here defined as the Key Binding JWT.
