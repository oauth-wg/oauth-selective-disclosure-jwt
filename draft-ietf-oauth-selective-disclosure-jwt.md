...
If any step fails, the SD-JWT is not valid, and processing MUST be aborted. Otherwise, the JSON document resulting from the preceding processing and verification steps, henceforth referred to as the processed SD-JWT payload, can be made available to the application to be used for its intended purpose.
...
# Document History
   ...
   * Introduced the phrase processed SD-JWT payload in the end of the (#sd_jwt_verification) on Verifying the SD-JWT
