...
The JSON-based content of signed JSON Web Token (JWT) as defined in
[@!RFC7515] is secured against modification using digital signatures. A consumer
of a JWT that has checked the document's signature can safely assume
that the content has not been modified.  However, anyone
receiving a JWT can read the actual content of the document if not encrypted.
 ...
 * A **SD-JWT** is a JWT [@!RFC7515], which can be signed as a JWS, that contains hashes
